...
router.put('/:id', async (req, res) => {
    ...
    if (!customer) return res.send(404).send('There are no customers with the given id.');
    res.send(customer);
});

router.delete('/:id', async (req, res) => {
    const customer = await Customer.findByIdAndRemove(req.params.id);
    if (!customer) return res.status(404).send('There are no customers with the given id.');
    res.send(customer);
});
